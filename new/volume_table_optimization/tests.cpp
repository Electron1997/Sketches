...
optimized_volume_table<uint32_t, float, D, W, hash32> optimized_table;
...
void test_optimized_table(int n){
    for(size_t i = 0; i < n; ++i){
        optimized_table.update(uniform_key<uint32_t>(0, 10000), normal_volume<float>(0.0, 5.0));
    }
}

int main(int argc, char *argv[]){
    int n = std::stoi(argv[1]);
    test_optimized_table(n);
}